...
export function generateTypeScript(
  ...
  return pipe(
    ...
    Task.ap(funcReturnType(types, stmt, options.camelCase)),
    Task.ap(Task.of(queryValues(stmt, positionalOnly))),
    Task.ap(Task.of(camelCaseMapper(stmt, options.camelCase))),
...
const typeScriptString = (
  ...
  (camelCasemapper: string) => (outputValue: string): string => `\
...
${sql}\`${queryValues})${camelCasemapper}
    return ${outputValue}
}
`

function camelCaseMapper(stmt: StatementDescription, returnCamelCase: boolean): string {
  if (stmt.rowCount === 'zero' || !returnCamelCase) {
    return '';
  }
  const columnAssignmentString = stmt.columns.map(col => {
    return `${camelCase(col.name)}: row.${col.name},`
  }).join('\n            ');
  return `
    result.rows = result.rows.map(row => {
        return {
            ${columnAssignmentString}
        };
    });`;
}
...
function funcReturnType(
  types: TypeClient,
  stmt: StatementDescription,
  returnCamelCase: boolean,
): Task.Task<string> {
  return pipe(
    traverseATs(stmt.columns, columnType(types, returnCamelCase)),
...
const columnType = (types: TypeClient, returnCamelCase: boolean) => (
  column: NamedValue
): Task.Task<string> => {
  return pipe(
    types.columnType(column),
    Task.map(({ name, type }) => `${stringLiteral(returnCamelCase ? camelCase(name) : name )}: ${type}`)
  )
}
